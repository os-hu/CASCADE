...
class MultiStageJavaTestGenerator(GPT4JavaTestGenerator):
    ...
    def build_prompt(self, context):
        ...
        system_prompt = f"You are a Java developer. You will generate unit tests for a specific method in a provided class.{testframework} You can import anything from the project, but no third party libraries. Handle exceptions properly, and ensure method signatures and calls are correct. The code should compile without errors."

        # This is for test driven development so the tests should be designed to fail if the later implementation does not conform to the documentation.
        ...
        test_header = f"\nNow write several tests regarding: {context["tested_property"]["property"]} for the function `{context['signature']['name']}({params})` using the test class skeleton below. Everything that you use has to be added to the imports. Every object you use has to be properly instantiated. Handle any checked exceptions (use try-catch or throws), do not forget type parameters. Match method signatures exactly when overriding or implementing methods. Adhere to the documentation as close as possible when writing the tests and try to test for the following things: \n{'\n'.join(context["tested_property"]["tests"])} \nAs a reminder, the documentation for the function is:\n\n```java\n{context['doc']}\n```\n\n Test class:\n"
    ...
    def generate(self, context, input_path, output_path):
        ...
        {"role": "user", "content": f"Give me properties that should be tested for the method:\n ```java\n{build_signature(context, doc=True)}\n```\nMake sure the entire functionality as described in the documentation is considered and covered with these tests."}
        ]
        ...
        prompt_stage1.append({"role": "user", "content": f"Now turn this into a JSON array. Where each entry has \"property\" (the tested property) and \"tests\" a list of things that should be tested to verify the property" })
        ...
        if json_blocks:
            try:
                tests_list = ast.literal_eval(json_blocks[0].strip())
        ...
        print(f"Got {len(tests_list)} potential test properties: {tests_list}")
        print("generation Phase 2")

        final_response = []
        final_new_tests = []

        for test in tests_list[0:3]:
            # now generate a test class for each property
            context["tested_property"] = test

            print("generate tests for:", test["property"])
            ...
            final_response.append({"property" : test["property"], "prompt" : prompt, "response" : response} )

            final_new_tests.append({ "new_tests" : new_tests , "property" : test["property"]})
