...
class MultiStepJavaTestGenerator(GPT4JavaTestGenerator):
    ...
    def build_prompt(self, context):
        ...
        system_prompt = (
            ...
            "You can import anything from the project, but no third party libraries. Handle exceptions properly, and ensure method signatures and calls are correct."
        ...
        c1 = (
            f"The function under test is `{context['signature']['name']}({params})`\n\n Only test: '{context["tested_property"]["property"]}', everything else will be tested later."
            f"Write tests for the following Things: {'\n'.join(context["tested_property"]["tests"])}\n"
            "Here is the class containing the function:\n\n```java\n"
            )

        code = c1 + build_context(context, doc=True) + "; // this is the function to be tested\n\n}\n```\n"
        ...
        test_header = (
            f"\nNow write tests regarding: {context['tested_property']['property']} for the function `{context['signature']['name']}({params})` using the test class skeleton below."
            ...
            f"Adhere to the documentation as close as possible when writing the tests and only test for the following things: \n{'\n'.join(context["tested_property"]["tests"])}"
    ...
    def generate(self, context, input_path, output_path):
        ...
        prompt_stage1.append({"role": "user",
                              "content": f"Now turn this into a JSON array of unit tests we should write for test driven development based on the documentation above. Where each entry has \"test_name\": referencing the specified behavior and \"test_description\": a more detailed description for the developer of what this tests should do and what behavior it tests."})
        ...
        final_response = [{"proplist": prop_list}]
        final_new_tests = []
        ...
        for prop in prop_list:
            ...
            if calls:
                ...
                new_test = self.extract_tests(repair_response["choices"][0]["message"]["content"], context,
                                              repair_response, output_path)
