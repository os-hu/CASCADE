...
def eval_cascade(gt, pd):
    ...
    if prediction == "Positive":
        results["no_phase_2"] = "TP" if gt else "FP"
    else:
        results["no_phase_2"] = "FN" if gt else "TN"


    if "INCO" in pd:
        results["Phase-2-no-p2f=0"] = "TP" if gt else "FP"
    else:
        results["Phase-2-no-p2f=0"] = "FN" if gt else "TN"
    ...
    if info["f2p"] > 0  and info["p2f"] == 0:
        results["cascade_full"] = "TP" if gt else "FP"
    else:
        results["cascade_full"] = "FN" if gt else "TN"
...
def eval_docchecker(gt, pd):
    ...
    split_predictions = pd.replace(" ", "").split(";")
    labels = ["asCodeis", "block", "singleLines"]
    results = {}
...
def evaluate_driver(driver):
    ...
    if driver == "CASCADE":
        ...
        eval = eval_docchecker     # dochchecker has the same format as c4rllama
    else:
        return
...
if __name__ == '__main__':
    ...
    driver_abbreviations = {"c": "CASCADE", "b": "Baseline", "d": "DocChecker", "4": "C4RLLaMA"}
